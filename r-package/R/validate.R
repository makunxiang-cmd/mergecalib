...
validate_merge_data <- function(data, targets = NULL, spec = merge_spec(), tolerance = 1e-8) {
  if (!is.data.frame(data) || !nrow(data)) {
    .mc_stop("mergecalib_error_input", "`data` must be a data frame with at least one row.")
  }
  req <- .required_columns(spec)
  missing_cols <- setdiff(req, names(data))
  if (length(missing_cols)) {
    .mc_stop("mergecalib_error_input", "`data` is missing required columns: ", paste(missing_cols, collapse = ", "), ".")
  }
  if (anyNA(data[[spec$id]]) || anyDuplicated(as.character(data[[spec$id]]))) {
    .mc_stop("mergecalib_error_input", "The cell identifier column `", spec$id, "` must be non-missing and unique.")
  }
  dim_cols <- .all_dimension_columns(spec)
  bad_dim <- dim_cols[vapply(data[dim_cols], anyNA, logical(1))]
  if (length(bad_dim)) {
    .mc_stop("mergecalib_error_input", "Demographic defining variables must not be missing: ", paste(bad_dim, collapse = ", "), ".")
  }
  ...
  if (anyDuplicated(dim_key)) {
    ...
    .mc_stop("mergecalib_error_input", "Each province-by-demographic combination must be unique; found ", length(dup), " duplicated cells.")
  }

  n <- data[[spec$n]]
  w <- data[[spec$weight]]
  if (!.is_integerish(n, tolerance) || any(n < 0)) {
    .mc_stop("mergecalib_error_input", "The sample-size column `", spec$n, "` must contain non-negative integers.")
  }
  if (!is.numeric(w) || any(!is.finite(w)) || any(w < 0)) {
    .mc_stop("mergecalib_error_input", "The weight column `", spec$weight, "` must contain finite non-negative numbers.")
  }
  if (any(n > 0 & w <= 0)) {
    bad <- data[[spec$id]][n > 0 & w <= 0]
    .mc_stop("mergecalib_error_input", "Positive-sample cells must have a positive total weight. Offending cells: ", paste(head(bad, 10), collapse = ", "), ".")
  }
  ...
  for (col in grade_cols) {
    ...
    if (!.is_integerish(x, tolerance) || any(x < 0)) {
      .mc_stop("mergecalib_error_input", "The grade-count column `", col, "` must contain non-negative integers.")
    }
  }
  ...
  if (any(bad_sum)) {
    ...
    .mc_stop("mergecalib_error_input", "The grade counts in each row must sum to the sample size. Offending cells: ", paste(head(bad, 10), collapse = ", "), ".")
  }
  if (any(n == 0 & grade_sum != 0)) {
    .mc_stop("mergecalib_error_input", "Cells with a sample size of 0 must have all grade counts equal to 0.")
  }
...
.validate_targets <- function(data, targets, spec, tolerance = 1e-8) {
  if (!is.data.frame(targets) || !nrow(targets)) {
    .mc_stop("mergecalib_error_target", "`targets` must be a data frame with at least one row.")
  }
  base_req <- c("target_id", "grade", "lower", "upper")
  miss <- setdiff(base_req, names(targets))
  if (length(miss)) {
    .mc_stop("mergecalib_error_target", "The targets table is missing columns: ", paste(miss, collapse = ", "), ".")
  }
  if (anyNA(targets$target_id) || any(trimws(as.character(targets$target_id)) == "")) {
    .mc_stop("mergecalib_error_target", "`target_id` must not be empty.")
  }
  key <- paste(targets$target_id, targets$grade, sep = "\r")
  if (anyDuplicated(key)) {
    .mc_stop("mergecalib_error_target", "Each combination of `target_id` and `grade` may appear only once.")
  }
  if (anyNA(targets$grade) || !all(as.character(targets$grade) %in% names(spec$grades))) {
    .mc_stop("mergecalib_error_target", "`grade` must be one of: ", paste(names(spec$grades), collapse = ", "), ".")
  }
  if (!is.numeric(targets$lower) || !is.numeric(targets$upper) ||
      any(!is.finite(targets$lower)) || any(!is.finite(targets$upper))) {
    .mc_stop("mergecalib_error_target", "`lower` and `upper` must be finite numbers.")
  }
  if (any(targets$lower < -tolerance | targets$upper > 1 + tolerance |
          targets$lower > targets$upper + tolerance)) {
    .mc_stop("mergecalib_error_target", "Target intervals must satisfy 0 <= lower <= upper <= 1.")
  }
  ...
  for (id in ids) {
    ...
    if (length(unique(signature)) != 1L) {
      .mc_stop("mergecalib_error_target", "All grades of the same target_id must describe the same target population. Offending target_id: ", id, ".")
    }
    if (length(unique(as.character(z$grade))) == length(spec$grades)) {
      if (sum(z$lower) > 1 + tolerance || sum(z$upper) < 1 - tolerance) {
        .mc_stop("mergecalib_error_target", "Target `", id, "` has intervals inconsistent with the requirement that the proportions sum to 1.")
      }
    }
  }
  ...
  for (i in seq_len(nrow(targets))) {
    ...
    if (!any(match)) {
      .mc_stop("mergecalib_error_target", "Target row ", i, " (", targets$target_id[i], ") did not match any original cell.")
    }
    if (sum(n[match]) <= 0) {
      .mc_stop(
